                                                            ...
                                                            const MahjongMode mode)
{
    ...
    if (type & ShantenFlag::Regular) {
        const auto [shanten, wait] = calc_regular(hand, num_melds, mode);
    ...
    if ((type & ShantenFlag::SevenPairs) && num_melds == 0) {
        const auto [shanten, wait] = calc_seven_pairs(hand, mode);
        if (shanten < std::get<1>(ret)) {
...
NecessaryTileCalculator::calc_seven_pairs(const Hand &hand, const MahjongMode mode)
{
    int num_pairs = 0;
    int num_types = 0;
