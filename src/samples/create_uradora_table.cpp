                  ...
                  std::mt19937 &engine)
{
    ...
    std::sample(yama.begin(), yama.end(), std::back_inserter(uradora_indicators), n, engine);
...
int main()
{
    ...
    std::vector<int> hand = {Tile::Manzu1, Tile::Manzu2, Tile::Manzu3, Tile::Manzu5, Tile::Manzu6,
                             Tile::Manzu7, Tile::Pinzu1, Tile::Pinzu2, Tile::Pinzu3, Tile::Pinzu6,
                             Tile::Pinzu7, Tile::Pinzu8, Tile::Sozu1,  Tile::Sozu1};
    ...
    // 裏ドラが乗った数
    int N = 10000000;
    std::vector<std::vector<double>> stats(6, std::vector<double>(13, 0)); //  (0~5枚, 0~12枚以上)
    for (int n = 1; n <= 5; ++n) {
        for (int i = 0; i < N; ++i) {
