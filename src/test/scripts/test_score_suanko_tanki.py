...
def is_suanko_tanki(hand, winning_tile):
    return sum([x == 3 for x in key]) == 4 and hand[winning_tile] == 2
...
for key in tqdm(product(range(5), repeat=9), total=5 ** 9):
    ...
    if shanten.calculate_shanten(hand) == -1:
        for winning_tile in np.unique(key):
            flatten = flatten_tile34(hand)
            is_established = is_suanko_tanki(hand, winning_tile)
